...
public class pageRanking {
	...
	public boolean rankPages(String searchQuery) throws Exception {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fileLexicon))) {
			...
			if (!ifWordExists) {
				System.out.println("\nUser entered Product that does not exist in directory");
				...
				if (rankedPages.size() == 0) {
					...
					System.out.println("Looking for occurances of the Product " + searchQuery);
					...
					System.out.println("Found total " + ttlOcrnce + " matches for the Product " + searchQuery + " in ("
							+ tt / 1000 + " seconds)");
					/*System.out.println(
